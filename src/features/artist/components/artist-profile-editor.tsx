...
export function ArtistProfileEditor({ artist }: Props) {
  ...
  async function uploadImage(event: ChangeEvent<HTMLInputElement>, kind: "PROFILE" | "COVER") {
    const file = event.target.files?.[0];
    if (!file) return;
    setUploading(kind); setError(null); setMessage(null);
    const form = new FormData(); form.set("file", file); form.set("kind", kind);
    try {
      const response = await fetch(`/api/artists/${artist.id}/media`, { method: "POST", body: form });
      const result: unknown = await response.json();
      if (!response.ok || typeof result !== "object" || result === null || !("url" in result) || typeof result.url !== "string") throw new Error(typeof result === "object" && result !== null && "error" in result && typeof result.error === "string" ? result.error : "Görsel yüklenemedi.");
      set(kind === "PROFILE" ? "profileImageUrl" : "coverImageUrl", result.url);
      setMessage("Görsel yüklendi. Kaydettiğinizde profilinizde yayınlanacak.");
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Görsel yüklenemedi."); }
  ...
  return <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:py-12">
    ...
    <form className="grid gap-5" onSubmit={submit}>
      <section className="panel grid gap-4 p-5 md:p-7"><div><h2 className="text-lg font-semibold">Kanal görünümü</h2><p className="mt-1 text-sm text-muted">Profil fotoğrafı ve kapak görselini doğrudan cihazınızdan yükleyin. JPG, PNG veya WebP desteklenir.</p></div><label className="grid gap-2 text-sm font-semibold">Sanatçı adı<input className="h-11 rounded-xl border border-line bg-transparent px-3 font-normal" value={values.name ?? ""} onChange={(e) => set("name", e.target.value)} required /></label><div className="grid gap-4 md:grid-cols-2"><div className="grid gap-3 rounded-2xl border border-line bg-surface-strong/40 p-4"><label className="grid gap-2 text-sm font-semibold">Profil fotoğrafı<input className="h-11 rounded-xl border border-line bg-transparent px-3 font-normal" accept="image/jpeg,image/png,image/webp" type="file" onChange={(event) => void uploadImage(event, "PROFILE")} disabled={uploading !== null} /></label><div className="flex items-center gap-3"><div className="grid size-16 shrink-0 place-items-center overflow-hidden rounded-2xl bg-accent/10 text-xl font-bold text-accent">{values.profileImageUrl ? <img alt="Profil fotoğrafı önizlemesi" className="size-full object-cover" onError={(event) => { event.currentTarget.style.display = "none"; }} src={values.profileImageUrl} /> : values.name?.slice(0, 1).toUpperCase()}</div><p className="text-xs text-muted">{uploading === "PROFILE" ? "Yükleniyor…" : "Dosya seçtiğinizde güvenli depolamaya yüklenir."}</p></div></div><div className="grid gap-3 rounded-2xl border border-line bg-surface-strong/40 p-4"><label className="grid gap-2 text-sm font-semibold">Kanal kapak fotoğrafı<input className="h-11 rounded-xl border border-line bg-transparent px-3 font-normal" accept="image/jpeg,image/png,image/webp" type="file" onChange={(event) => void uploadImage(event, "COVER")} disabled={uploading !== null} /></label><div className="h-16 overflow-hidden rounded-xl bg-gradient-to-r from-accent/30 to-surface-strong">{values.coverImageUrl ? <img alt="Kapak fotoğrafı önizlemesi" className="size-full object-cover" onError={(event) => { event.currentTarget.style.display = "none"; }} src={values.coverImageUrl} /> : <p className="p-5 text-xs text-muted">{uploading === "COVER" ? "Yükleniyor…" : "Kapak fotoğrafı önizlemesi"}</p>}</div></div></div><label className="grid gap-2 text-sm font-semibold">Kanal URL slug<input className="h-11 rounded-xl border border-line bg-transparent px-3 font-normal" pattern="[a-z0-9]+(?:-[a-z0-9]+)*" value={values.slug ?? ""} onChange={(e) => set("slug", e.target.value)} required /></label><label className="grid gap-2 text-sm font-semibold">Kısa açıklama<textarea className="min-h-24 rounded-xl border border-line bg-transparent p-3 font-normal" maxLength={500} value={values.shortBiography ?? ""} onChange={(e) => set("shortBiography", e.target.value)} /></label><label className="grid gap-2 text-sm font-semibold">Kanal hakkında<textarea className="min-h-36 rounded-xl border border-line bg-transparent p-3 font-normal" value={values.biography ?? ""} onChange={(e) => set("biography", e.target.value)} /></label></section>
