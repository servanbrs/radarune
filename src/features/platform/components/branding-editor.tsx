...
function applyFavicon(url: string | null | undefined) {
  ...
  const iconUrl = `${url}${url.includes("?") ? "&" : "?"}v=${Date.now()}`;
...
export function BrandingEditor() {
  ...
  const { register, reset, setValue, handleSubmit, formState: { isSubmitting, errors } } = useForm<BrandingFormInput, unknown, BrandingUpdateInput>({ resolver: zodResolver(brandingUpdateSchema), defaultValues: { brandName: "", legalName: null, logoUrl: null, faviconUrl: null, supportEmail: null } });
  ...
  return <form className="panel grid gap-5 p-6" onSubmit={handleSubmit(save, () => setMessage("Lütfen kırmızı görünen alanları düzeltin."))}>{message ? <p className="rounded-2xl border border-line bg-surface-strong p-3 text-sm">{message}</p> : null}<div className="grid gap-4 md:grid-cols-2"><label className="grid gap-2 text-sm font-medium">Marka adı<Input {...register("brandName")} />{fieldError("brandName")}</label><label className="grid gap-2 text-sm font-medium">Yasal ad<Input {...register("legalName")} />{fieldError("legalName")}</label><label className="grid gap-2 text-sm font-medium">Logo dosyası<input className="h-11 rounded-xl border border-line bg-transparent px-3 py-2 text-sm font-normal" accept="image/jpeg,image/png,image/webp" type="file" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadMedia(file, "LOGO").catch((error: unknown) => setMessage(error instanceof Error ? error.message : "Logo yüklenemedi.")); }} /><input type="hidden" {...register("logoUrl")} /><span className="text-xs font-normal text-muted">İdeal ölçü: 512x512 piksel. Şeffaf PNG önerilir; en az 256x256 olmalıdır.</span>{fieldError("logoUrl")}</label><label className="grid gap-2 text-sm font-medium">Favicon dosyası<input className="h-11 rounded-xl border border-line bg-transparent px-3 py-2 text-sm font-normal" accept="image/png,image/x-icon" type="file" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadMedia(file, "FAVICON").catch((error: unknown) => setMessage(error instanceof Error ? error.message : "Favicon yüklenemedi.")); }} /><input type="hidden" {...register("faviconUrl")} /><span className="text-xs font-normal text-muted">İdeal ölçü: 32x32 veya 48x48 piksel. PNG/ICO; 32–512 piksel aralığı.</span>{fieldError("faviconUrl")}</label><label className="grid gap-2 text-sm font-medium">Destek e-postası<Input type="email" {...register("supportEmail")} />{fieldError("supportEmail")}</label></div><Button disabled={loading || isSubmitting} type="submit">{loading ? "Ayarlar yükleniyor…" : isSubmitting ? "Kaydediliyor…" : "Kaydet"}</Button></form>;
}
