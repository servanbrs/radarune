...
export function ImportSourceCreateForm() {
  ...
  return (
    <section className="panel p-6">
      ...
      <p className="mt-2 text-sm text-muted">URL ile kaynak ekleyin veya arama seçeneğiyle güncel müziklerden 100–200 içeriği toplu çekin. Her içerik önce inceleme kuyruğuna alınır.</p>
      <div className="mt-5 grid gap-3 md:grid-cols-[220px_1fr_1fr_140px_auto]">
        ...
        {type.endsWith("SEARCH") ? <input className="rounded-xl border border-line bg-surface-strong px-3 py-2 text-sm" onChange={(event) => setQuery(event.target.value)} placeholder="Arama terimi: yeni türkçe müzik" value={query} /> : <input className="rounded-xl border border-line bg-surface-strong px-3 py-2 text-sm" onChange={(event) => setUrl(event.target.value)} placeholder={type === "YOUTUBE_CHANNEL" ? "https://youtube.com/@kanal veya /channel/UC…" : type.startsWith("YOUTUBE") ? "https://youtube.com/playlist?list=…" : "https://open.spotify.com/..."} type="url" value={url} />}
      ...
      <label className="mt-4 flex items-center gap-2 text-sm text-muted"><input checked={autoPublish} onChange={(event) => setAutoPublish(event.target.checked)} type="checkbox" /> Güvenilir kaynaklarda otomatik kabul et (inceleme kuyruğunu atlar)</label>
