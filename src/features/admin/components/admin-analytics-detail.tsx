    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
        <Panel eyebrow="Canlı oturumlar" title="Aktif kullanıcılar ve maskeli IP görünümü" icon={Users}>
          ...
          <div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left text-sm"><thead className="border-b border-line text-xs text-muted"><tr><th className="pb-3">Kullanıcı</th><th className="pb-3">Ülke</th><th className="pb-3">Şehir</th><th className="pb-3">IP</th><th className="pb-3">Cihaz</th><th className="pb-3">Son hareket</th></tr></thead><tbody className="divide-y divide-line">{dashboard.details.activeSessions.map((session) => <tr key={session.id}><td className="py-3"><Link className="font-semibold hover:text-[#d6a85f]" href={`/admin/users/${session.userId}`}>{session.name}</Link><span className="mt-1 block text-xs text-muted">{session.email}</span></td><td className="py-3 text-xs">{session.country}</td><td className="py-3 text-xs">{session.city}</td><td className="py-3 font-mono text-xs">{session.ipAddress}</td><td className="max-w-[220px] truncate py-3 text-xs text-muted">{session.userAgent}</td><td className="py-3 text-xs text-muted">{new Date(session.updatedAt).toLocaleTimeString("tr-TR")}</td></tr>)}</tbody></table></div>
