...
type SpotifyArtist = { id: string; name: string; url: string; imageUrl: string | null; followers: number; popularity: number };
...
export function ArtistApplicationForm() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      setSearchError(null);
      try {
        const response = await fetch(`/api/integrations/spotify/artists?query=${encodeURIComponent(stageName.trim())}`, { cache: "no-store", signal: controller.signal });
        const payload = (await response.json()) as { items?: SpotifyArtist[]; error?: string };
  ...
  return (
    <form action={submit} className="mt-8 grid gap-4">
      <div className="grid gap-2 text-sm font-medium">
        ...
        {results.length ? <div className="grid gap-2 rounded-2xl border border-line bg-surface p-2">{results.map((artist) => <button className="flex items-center gap-3 rounded-xl p-2 text-left hover:bg-surface-strong" key={artist.id} onClick={() => chooseArtist(artist)} type="button">{artist.imageUrl ? <img alt="" className="size-10 rounded-full object-cover" src={artist.imageUrl} /> : <span className="grid size-10 place-items-center rounded-full bg-accent/10 text-accent">♪</span>}<span className="min-w-0 flex-1"><span className="block truncate font-semibold">{artist.name}</span><span className="block text-xs text-muted">{artist.followers.toLocaleString("tr-TR")} takipçi · Spotify</span></span></button>)}</div> : null}
        ...
        {selectedArtist ? <div className="flex items-center justify-between gap-3 rounded-xl border border-accent/40 bg-accent/10 p-3 text-xs"><span><strong>{selectedArtist.name}</strong> Spotify profili seçildi.</span><button className="font-semibold text-accent" onClick={() => setSelectedArtist(null)} type="button">Değiştir</button></div> : stageName.trim().length >= 2 && !searching && !results.length && !searchError ? <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-dashed border-line p-3 text-xs"><span>Spotify’da eşleşme yok. Yeni sanatçı olarak devam edebilirsiniz.</span><Link className="font-semibold text-accent" href="/dashboard/support?subject=Spotify%20sanat%C3%A7%C4%B1%20profili%20ekleme">Destek ekibine yaz</Link></div> : null}
        <input name="spotifyArtistUrl" type="hidden" value={selectedArtist?.url ?? ""} />
      </div>
