...
export function DiscoverLikeButton({ trackId, releaseId }: { trackId?: string | null; releaseId?: string | null }) {
  const [state, setState] = useState<"idle" | "liked" | "error">("idle");
  async function like() {
    if (state === "liked") return;
    const response = await fetch("/api/growth/like", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(trackId ? { trackId } : { releaseId }) });
