      ...
      <div className="pointer-events-auto fixed inset-x-3 bottom-3 z-[2147483647] rounded-2xl border border-white/15 bg-[#111827] px-4 py-3 text-white shadow-[0_-12px_40px_rgba(0,0,0,0.4)] sm:inset-x-5">
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-3">
          <p className="text-xs text-white/70 sm:text-sm">
            Web player hazır — keşfet akışından bir şarkı seçin.
          </p>
      ...
      <div className="mx-auto flex max-w-[1500px] flex-wrap items-center gap-2.5 sm:flex-nowrap sm:gap-4">
        <div className="min-w-0 flex-1 basis-[calc(100%-2.75rem)] sm:basis-auto">
          ...
          <p className="truncate text-xs text-white/60">
            {item.artistName} · {item.sourceLabel} · {index + 1}/
            {queue.length || 1}
          </p>
          {error ? (
            <p className="truncate text-[11px] text-red-400" role="status">
              {error}
            </p>
          ) : null}
        </div>
        <button
          aria-label="Playerı kapat"
          className="order-1 rounded-full p-2 text-white/70 hover:text-white sm:order-none"
          onClick={close}
          type="button"
        >
          <X className="size-4" />
        ...
        <div className="flex w-full min-w-0 items-center justify-end gap-2 sm:w-auto sm:shrink-0 sm:justify-start">
          ...
          <button
            aria-label="Önceki şarkı"
            className="shrink-0 rounded-full border border-white/20 p-2"
            onClick={previous}
            type="button"
          >
            <SkipBack className="size-4" />
          ...
          <button
            ...
            className="shrink-0 rounded-full bg-foreground p-3 text-white disabled:opacity-40"
          ...
          {item.playbackUrl ? (
            <input
              ...
              className="hidden w-28 accent-[var(--accent)] md:block"
              max={duration || 0}
              min="0"
