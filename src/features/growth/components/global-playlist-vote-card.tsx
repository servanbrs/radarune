...
type GlobalPlaylistVoteCardProps = {
  playlist: {
    ...
    featured: boolean;
    tracks: Array<{ track: { id: string; title: string; artists: Array<{ artist: { id: string; name: string; slug: string } }> }; release: { title: string } | null }>;
...
export function GlobalPlaylistVoteCard({ playlist }: GlobalPlaylistVoteCardProps) {
  ...
  return <article className="rounded-[1.5rem] border border-line bg-surface p-5">
    ...
    <div className="mt-5 grid gap-2">{playlist.tracks.slice(0, 5).map((item, index) => <div className="flex items-center gap-3 rounded-xl border border-line/70 px-3 py-2 text-sm" key={item.track.id}><span className="font-mono text-xs text-muted">{String(index + 1).padStart(2, "0")}</span><div className="min-w-0"><p className="truncate font-medium">{item.track.title}</p>{item.track.artists[0] ? <Link className="text-xs text-muted hover:text-accent" href={`/artist/${item.track.artists[0].artist.slug}`}>{item.track.artists[0].artist.name}</Link> : null}</div><span className="ml-auto shrink-0 text-xs text-muted">{item.release?.title ?? "Yayın"}</span></div>)}{playlist.tracks.length === 0 ? <p className="rounded-xl border border-dashed border-line p-4 text-sm text-muted">Bu playlist henüz parça bekliyor.</p> : null}</div>
