...
export function SupportInbox({ admin = false }: { admin?: boolean }) {
  ...
  return <div className="space-y-5">
    <section className="panel flex flex-wrap items-center justify-between gap-3 p-4">
      ...
      <div className="flex flex-wrap gap-2">{activeStaff.length ? activeStaff.map((member) => <span className="inline-flex items-center gap-2 rounded-full border border-line bg-surface-strong px-3 py-1.5 text-xs" key={member.id}><span className="h-2 w-2 rounded-full bg-emerald-400" />{member.name} · {roleLabel(member.role)}</span>) : <span className="rounded-full border border-line px-3 py-1.5 text-xs text-muted">Şu anda çevrimiçi ekip üyesi yok.</span>}</div>
    </section>
