...
const quickSignUpSchema = z.object({
  ...
  acceptTerms: z
    .boolean()
    .refine(Boolean, "Kullanım koşullarını kabul etmelisiniz."),
});
...
export function QuickSignUpForm() {
  ...
  const form = useForm<QuickSignUpValues>({
    ...
    <form className="grid gap-4" onSubmit={onSubmit}>
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="grid gap-2 text-sm font-medium text-white/80">
          Ad soyad
          <input
            className="h-11 rounded-xl border border-white/15 bg-black/20 px-3 text-white outline-none placeholder:text-white/35 focus:border-[#efb848]"
      ...
      <label className="flex items-start gap-3 text-xs leading-5 text-white/60">
        <input
          className="mt-1 size-4 accent-[#efb848]"
          type="checkbox"
        ...
        <span>
          ...
          {form.formState.errors.acceptTerms ? (
            <span className="mt-1 block text-[#ff9b9b]">
      ...
      {error ? (
        <p className="rounded-xl border border-[#ff9b9b]/30 bg-[#ff9b9b]/10 px-3 py-2 text-xs text-[#ffb2b2]">
          {error}
        </p>
      ) : null}
      <button
        className="inline-flex h-12 items-center justify-center rounded-xl bg-[#efb848] px-5 font-semibold text-[#090b0f] transition hover:bg-[#ffd46f] disabled:cursor-wait disabled:opacity-60"
        disabled={pending}
        type="submit"
      >
