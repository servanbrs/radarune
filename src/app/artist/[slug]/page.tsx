        ...
        <div className="grid gap-8 p-6 md:p-10 lg:grid-cols-[1.5fr_1fr]">
          <div>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {artist.releaseArtistLinks.map((link) => <div className="overflow-hidden rounded-2xl border border-line bg-surface-strong" key={link.id}><div className="aspect-square bg-gradient-to-br from-accent/30 to-surface-strong">{link.release.artworkUploadId ? <img alt={`${link.release.title} kapak`} className="h-full w-full object-cover" src={`/api/releases/${link.release.id}/artwork`} /> : null}</div><div className="p-4"><p className="font-semibold">{link.release.title}</p><p className="mt-1 text-xs text-muted">{link.release.primaryGenre}</p></div></div>)}
