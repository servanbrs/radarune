    ...
    <PublicGrowthShell currentUser={session ? { name: session.user.name } : null}>
      ...
      <section className="panel overflow-hidden p-0">
        ...
        <div className="relative flex flex-wrap items-end gap-6">
            ...
            <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="text-xs uppercase tracking-[0.24em] text-accent">Radarune artist channel</p><span className="inline-flex items-center gap-1 rounded-full bg-emerald-300/15 px-2.5 py-1 text-[10px] font-bold text-emerald-800"><span className="size-1.5 rounded-full bg-emerald-500" />Radarune sanatçı profili</span></div><h1 className="mt-2 text-4xl font-semibold md:text-5xl">{artist.name}</h1><p className="mt-2 text-sm text-muted">{artist.type === "SOLO" ? "Solo sanatçı" : artist.type} · herkese açık sanatçı kanalı</p><div className="mt-4 flex flex-wrap gap-2"><DiscoverArtistFollowButton artistId={artist.id} initialFollowing={initialFollowing} isAuthenticated={Boolean(session)} /><ArtistProfileShareButton slug={artist.slug} /></div></div>
          </div>
        </div>
