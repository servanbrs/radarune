...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const track = await prisma.track.findFirst({ where: { id, organizationId: access.tenant.id, release: { status: "LIVE" } }, select: { id: true, title: true, versionTitle: true, trackNumber: true, discNumber: true, language: true, explicit: true, instrumental: true, isrc: true, durationMs: true, release: { select: { id: true, title: true, type: true } }, artists: { select: { role: true, artist: { select: { id: true, name: true, slug: true } } } } } });
