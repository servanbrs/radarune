...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [items, total] = await Promise.all([
      prisma.release.findMany({ where: { organizationId: access.tenant.id, status: { in: ["LIVE", "DISTRIBUTED"] } }, orderBy: { liveAt: "desc" }, skip: (page - 1) * pageSize, take: pageSize, select: { id: true, title: true, versionTitle: true, primaryLanguage: true, primaryGenre: true, type: true, explicit: true, plannedReleaseDate: true, originalReleaseDate: true, upc: true, liveAt: true, artists: { select: { role: true, artist: { select: { id: true, name: true, slug: true } } } }, _count: { select: { tracks: true } } } }),
      prisma.release.count({ where: { organizationId: access.tenant.id, status: { in: ["LIVE", "DISTRIBUTED"] } } }),
    ]);
