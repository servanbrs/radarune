...
export async function GET() {
  try {
    ...
    const notifications = await prisma.notification.findMany({ where: { userId: session.user.id, OR: [{ organizationId: organization.organization.id }, { organizationId: null }] }, orderBy: { createdAt: "desc" }, take: 30, select: { id: true, type: true, title: true, message: true, readAt: true, createdAt: true } });
