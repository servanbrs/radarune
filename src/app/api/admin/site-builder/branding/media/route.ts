...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await publicMediaService.upload({ file, organizationId: actor.organizationId, ownerId: actor.userId, kind: kind as PublicMediaKind, publicBaseUrl: new URL(request.url).origin }));
  } catch (error) {
