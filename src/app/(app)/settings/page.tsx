...
export default async function SettingsPage() {
  ...
  return <main className="page-shell pb-24">
    ...
    <nav className="mt-6 grid gap-3 sm:grid-cols-4" aria-label="Hesap kısayolları">
      {[{ href: "#profile", label: "Profil", note: "Kişisel bilgiler" }, { href: "#security", label: "Güvenlik", note: "Şifre ve oturum" }, { href: "/playlists", label: "Playlistler", note: "Müzik kütüphanen" }, { href: "/smart-links", label: "Smart Link", note: "Paylaşım araçları" }].map((item) => <Link className="rounded-2xl border border-line bg-surface p-4 transition hover:-translate-y-0.5 hover:border-accent" href={item.href} key={item.href}><span className="block text-sm font-bold">{item.label}</span><span className="mt-1 block text-xs text-muted">{item.note}</span></Link>)}
    </nav>

    <div className="mt-6 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
      <div className="grid content-start gap-6">
        <section className="panel p-6" id="profile"><p className="text-xs uppercase tracking-[0.2em] text-muted">Profil</p><h2 className="mt-2 text-xl font-semibold">{user.name}</h2><p className="mt-1 text-sm text-muted">{user.email}</p><div className="mt-5 grid gap-3 rounded-2xl border border-line bg-background/50 p-4 text-sm"><div className="flex justify-between gap-3"><span className="text-muted">Kullanıcı adı</span><span className="font-medium">Profil sayfasından yönetilir</span></div><div className="flex justify-between gap-3"><span className="text-muted">Katılım</span><span className="font-medium">{created}</span></div></div><Link className="mt-5 inline-flex rounded-full bg-accent px-5 py-3 text-sm font-semibold text-accent-foreground" href="/artist-profile/edit">Profili düzenle →</Link></section>
