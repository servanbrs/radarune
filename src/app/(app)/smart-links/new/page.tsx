...
export default async function NewSmartLinkPage() {
  ...
  return (
    <main className="page-shell">
      ...
      {artists.length > 0 ? <SmartLinkForm artists={artists.map((artist) => ({ id: artist.id, name: artist.name }))} /> : <section className="panel p-6"><p className="font-semibold">Önce bir sanatçı profili oluşturun.</p></section>}
    </main>
  );
}
