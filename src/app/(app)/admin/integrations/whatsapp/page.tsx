...
export default async function AdminWhatsappPage() {
  ...
  return (
    <AdminShell title="WhatsApp bildirimleri" description="Yeni yayın geldiğinde admin numaralarına Meta WhatsApp Cloud API üzerinden şablon mesaj gönderin.">
      <section className="panel p-6">
        ...
        <form action={saveWhatsappAction} className="mt-6 grid gap-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="grid gap-2 text-sm font-medium md:col-span-2">Meta doğrulama belirteci<input className="rounded-xl border border-line bg-background px-4 py-3" defaultValue={config?.verifyToken ?? "radarune_whatsapp_verify"} name="verifyToken" pattern="[ -~]+" title="Yalnızca İngilizce harf, rakam ve temel semboller kullanılabilir." required /><span className="text-xs font-normal text-muted">Meta Webhooks ekranındaki “Belirteci doğrula” alanına bu değeri aynen yazın. Türkçe karakter kullanmayın.</span></label>
          </div>
      ...
      <section className="panel mt-5 p-6"><h2 className="text-lg font-semibold">Meta Webhooks bağlantısı</h2><div className="mt-4 grid gap-3 text-sm leading-6 text-muted"><p><strong className="text-foreground">Geri arama URL’si:</strong> <code className="break-all rounded bg-surface-strong px-2 py-1">https://radarune.com/api/webhooks/whatsapp</code></p><p><strong className="text-foreground">Doğrulama belirteci:</strong> Yukarıdaki alanda kayıtlı değer.</p><p>Meta ekranında üretim webhook’larının çalışması için radarune.com alan adının HTTPS üzerinden erişilebilir ve uygulamanın yayınlanmış olması gerekir. Localhost Meta tarafından erişilemez.</p></div></section>
