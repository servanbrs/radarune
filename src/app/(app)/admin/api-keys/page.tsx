...
import { ApiKeyCreateForm } from "@/features/admin/components/api-key-create-form";
...
export default async function AdminApiKeysPage() {
  ...
  return <AdminShell title="Public API anahtarları" description="Anahtarların hash değeri saklanır; tam anahtar yalnızca oluşturma yanıtında bir kez gösterilir."><ApiKeyCreateForm /><section className="panel p-6"><SimpleTable columns={["Ad", "Prefix", "Scope", "Son kullanım", "Durum"]} rows={keys.map((key) => [key.name, key.prefix, Array.isArray(key.scopes) ? key.scopes.join(", ") : "", key.lastUsedAt?.toLocaleString("tr-TR") ?? "Henüz kullanılmadı", key.revokedAt ? "İptal edildi" : "Aktif"])} /></section></AdminShell>;
}
