...
export default async function PlaylistsPage() {
  ...
  const playlists = await socialRepository.listPlaylistsForViewer(organization.organization.id, user.id);
  ...
  return (
    <main className="page-shell">
      ...
      <section className="panel p-6 md:p-8"><p className="text-xs uppercase tracking-[0.24em] text-accent">Kütüphanen</p><h2 className="mt-2 text-2xl font-semibold">Playlistlerin</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted">Yayınlanmış Radarune parçalarını topla, gizli taslak olarak sakla veya herkese açık bir keşif listesi yayınla.</p></section>
      <section className="grid gap-4 md:grid-cols-2">
        {playlists.map((playlist) => (
          <Link className="panel block p-5" href={playlist.slug ? `/playlist/${playlist.slug}` : `/playlists/${playlist.id}`} key={playlist.id}>
            ...
            <p className="mt-1 text-sm text-muted">{playlist.ownerUser.name} · {playlist.tracks.length} parça · {playlist.public ? "Herkese açık" : "Gizli"}</p>
          </Link>
        ))}
      </section>
    </main>
