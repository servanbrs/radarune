...
export default async function PlaylistDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const availableTracks = await prisma.track.findMany({ where: { organizationId: organization.organization.id, release: { status: { in: ["APPROVED", "DISTRIBUTED", "LIVE"] } } }, orderBy: { createdAt: "desc" }, take: 300, select: { id: true, title: true, release: { select: { title: true } }, artists: { orderBy: { sortOrder: "asc" }, select: { artist: { select: { name: true } } } } } });
